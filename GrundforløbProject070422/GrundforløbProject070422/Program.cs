...
{
    Console.WriteLine(FindMinMax(12, 12, 3, 4));
    Console.Write("Test");
}

static double[] FindMinMax(params double[] values)
{
    double maks = 0;
    double minst = 100;
    for (int i = 0; i < values.Length; i++)
    {
        if (values[i] > maks) { maks = values[i]; }
        if (values[i] < minst) { minst = values[i]; }
    }
    double[] final = new double[] { minst, maks };
    return final;
